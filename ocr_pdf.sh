...
if [ "$1" != "--batch" ]; then
    ...
    "$OCRMYPDF" --language dan --skip-text --deskew --optimize 1 --invalidate-digital-signatures "$INPUT" "$OUTPUT"
    echo "✓ Done: $OUTPUT"
...
# Batch processing
    ...
    for pdf in "$DIR"/*.pdf "$DIR"/*.PDF; do
        if [ -f "$pdf" ]; then
            ...
            ERROR_OUTPUT=$("$OCRMYPDF" --language dan --skip-text --deskew --optimize 1 "$pdf" "$output" 2>&1)
